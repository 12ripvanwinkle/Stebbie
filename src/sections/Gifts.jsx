...
const Gifts = () => {
    ...
    const panelRef   = useScrollAnimation({ threshold: 0.15 })
    ...
    <section className="py-24 relative overflow-hidden" id='gifts'>
      ...
      <div className="max-w-2xl mx-auto px-6">
        ...
        <div className="text-center mb-14">
          ...
          <p ref={subtitleRef} className="fade-up delay-300 font text-white mt-3 text-sm max-w-md mx-auto leading-relaxed">
            Your presence is our greatest gift. But Your Money is even Greater. 💕
          </p>
        </div>

        {/* ── Gift Panel ── */}
        <div ref={panelRef} className="fade-up delay-400 bg-gradient-to-br from-lilac-50 to-babypink-50 rounded-3xl shadow-lg p-8 border border-lilac-100">
          ...
          {method && (
            <div className="fade-in animate-in bg-white rounded-2xl p-5 border border-lilac-100 mb-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font text-xs text-gray-400 uppercase tracking-wider mb-1">
                    {payDetails[method].label} Handle
                  </p>
                  <p className="font text-lg font-bold text-lilac-700">{payDetails[method].value}</p>
                  {finalAmount && (
                    <p className="font text-xs text-babypink-400 mt-1">
                      Sending: <strong>${finalAmount}</strong>
                    </p>
                  )}
                </div>
                <button
                  onClick={() => copyToClipboard(payDetails[method].value, method)}
